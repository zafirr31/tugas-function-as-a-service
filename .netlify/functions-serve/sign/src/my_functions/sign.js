...
// my_functions/sign.ts
var convertAPIEventToSignatureParameters = (event) => {
...
var validateSignatureParameters = (event) => {
  return (0, import_function.pipe)(paramsNotNull(event), (0, import_Either.chain)(paramsArePositiveIntegers));
};
...
var calculateSignature = (event) => {
  return (0, import_Either2.right)(bytesToLong(fastModularExponentiation(BigInt(event.hash), BigInt(event.d), BigInt(event.n)).toString()));
};
var rightLongToBytes = (event) => {
  return (0, import_Either2.right)(longToBytes(event));
};
var sign = (event) => {
  return (0, import_function.pipe)(rightCalculateHash(event), (0, import_Either.chain)(calculateSignature), (0, import_Either.chain)(rightLongToBytes));
};
async function handler(event, context) {
  return (0, import_function.pipe)(validateSignatureParameters(event), (0, import_Either.chain)(convertAPIEventToSignatureParameters), (0, import_Either.chain)(sign), (0, import_Either.fold)((error) => errorResponse(error), (result) => successResponse(StatusCodes.OK, result)));
}
