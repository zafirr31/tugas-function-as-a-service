...
// my_functions/decrypt.ts
var convertAPIEventToDecryptionParameters = (event) => {
...
var validateDecryptionParameters = (event) => {
  return (0, import_function.pipe)(paramsNotNull(event), (0, import_Either.chain)(paramsArePositiveIntegers));
};
var calculateMessage = (event) => {
  return (0, import_Either2.right)(fastModularExponentiation(BigInt(event.c), BigInt(event.d), BigInt(event.n)));
};
var rightLongToBytes = (event) => {
  return (0, import_Either2.right)(longToBytes(event));
};
var decrypt = (event) => {
  return (0, import_function.pipe)(calculateMessage(event), (0, import_Either.chain)(rightLongToBytes));
};
async function handler(event, context) {
  return (0, import_function.pipe)(validateDecryptionParameters(event), (0, import_Either.chain)(convertAPIEventToDecryptionParameters), (0, import_Either.chain)(decrypt), (0, import_Either.fold)((error) => errorResponse(error), (result) => successResponse(StatusCodes.OK, result)));
}
